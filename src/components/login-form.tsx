...
export function LoginForm() {
  ...
  const acceptInviteAfterLogin = async () => {
    ...
    if (!res.ok) {
      ...
      throw new Error(body.error ?? "Failed to accept household invite");
    }
  };
      ...
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-slate-900 p-8 shadow-2xl">
        ...
        {inviteToken && (
          <div className="mb-6 rounded-xl border border-indigo-500/30 bg-indigo-500/10 p-4 text-sm text-indigo-100">
            You have been invited to join a household. Sign in or create an account with the
            invited email to connect your finances.
          </div>
        )}
